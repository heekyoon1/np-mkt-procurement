...
function detailView() {
  const r = activeRequest(); const item = r.items[0]; const prices = calcPrices(item.cost, item.category); const role = state.role; const canChange = ["buyer", "admin"].includes(role); const canApprove = ["lead", "admin"].includes(role) && r.approval === "대기"; const stepIndex = statusSteps.indexOf(r.status); const priceVisible = ["buyer", "admin", "lead"].includes(role);
